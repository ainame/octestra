...
import type { ListedIssue, ListedIssues } from "../shared/github-client";
...
// P6: a matrix cannot exceed 256 jobs, so a loop can never select more than that
// however generous select.limit is.
const matrixJobLimit = 256;
...
export function parseLoopContext(raw: string): LoopContext {
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("loop-context must contain loop_id");
  }
  const context = value as Record<string, unknown>;
  if (typeof context.loop_id !== "string" || !context.loop_id) {
    throw new Error("loop-context must contain loop_id");
  }
  return { loop_id: context.loop_id, dry_run: context.dry_run === true };
}

export type LoopSelection = PromptLoopIssue;

export function parseLoopSelection(raw: string): LoopSelection[] {
  const value: unknown = JSON.parse(raw);
  if (!Array.isArray(value)) {
    throw new Error("loop-issues must be a JSON array");
  }
  return value.map((entry, index) => {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
      throw new Error(`loop-issues[${index}] must be an object`);
    }
    ...
    if (
      ...
      throw new Error(`loop-issues[${index}].number must be a positive integer`);
    }
    return {
      number: issue.number,
...
export function resolveLoopBranchName(
  template: string,
  loopId: string,
  runNumber: string,
): string {
  if (!runNumber) {
    throw new Error("loop branch name requires a run number");
  }
  return renderBranchTemplate(
    template,
    { loop_id: loopId, run_number: runNumber },
    "config.yml branch.loop must include {loop_id} and {run_number}",
  );
}

function emitSelection(issues: LoopSelection[], partial: boolean): void {
...
function filteredCandidates(
  ...
  const cutoff = config.select.updated_before === undefined
    ? undefined
    : now.getTime() - config.select.updated_before * 1000;
  return issues
    .filter((candidate) => !candidate.pull_request)
    .filter((candidate) => cutoff === undefined || Date.parse(candidate.updated_at) < cutoff)
    .sort((left, right) => config.select.order === "oldest"
...
export async function selectTasks(
  ...
  const listed = config.select.epic === null
    ? await client.listIssues(config.select.labels, config.select.scan_budget)
    : await client.listSubIssues(config.select.epic, config.select.scan_budget);
...
export async function prepareRun(
  ...
  const scope = options.issueNumber === undefined
    ? loop.loop_id
    : `${loop.loop_id}-${options.issueNumber}`;
  ...
  if (options.issueNumber === undefined) {
    ...
    const branchName = resolveLoopBranchName(
      options.branchTemplate ?? defaultLoopBranchTemplate,
      loop.loop_id,
      options.runNumber ?? "",
    );
...
export async function finalizeRun(
  ...
  if (!target) {
    throw new Error("loop/finalize-run needs an issue-number or a configured report_issue");
  }
  ...
  if (config.apply.assign_owner) {
    ...
    if (!owner) {
      throw new Error(`Loop cannot promote issue #${issueNumber}: no task owner is assigned`);
    }
...
export async function reportFailure(
  ...
  const message = `Octestra loop ${loop.loop_id} failed: ${workflowRunUrl()}`;
...
export function loopDefinition(config: OctestraConfig, loop: LoopContext): LoopConfig {
  ...
  if (!definition) {
    throw new Error(`Unknown loop: ${loop.loop_id}`);
  }
  return definition;
}
