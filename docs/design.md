...
| D13 | A passed task PR is opened ready for review, is taken out of draft before review is requested, and gets no assignee; a validation-blocked PR assigns the task owner without requesting review; the EPIC opts out with `draft_pr: true` and out of validation with `skip_validation: true`, both defaulting to `false` | Every default here is the state a human wants at the moment they are asked to look. Marking a PR ready by hand is friction repeated on every task, and where validation runs the work has already been checked before a human sees it — so *ready* is the honest state, and requesting review on something GitHub labels a draft is a contradiction rather than a workflow. A passed PR sends a review-request notification, including integrations such as Slack, so assigning its owner as well would duplicate mobile notifications. A blocked PR instead needs a person to resolve it: assigning the task owner makes it discoverable with `assignee:@me` while avoiding a misleading review request. `skip_validation` is spelled as the exception it is: the negative name (`validation_required: false`) read as the normal case while describing the opt-out, and inverting it puts "run validation" in the default. |
| D14 | The installed workflow is replaced in full; consumer-owned composite actions, prompts, loop workflows, and `config.yml` are preserved in full; `octestra.sh update` shows changelog entries and drives the process by running the target version's own `install.sh` | The lifecycle workflow is mechanism that Octestra must be able to update coherently. Agent actions and prompts are policy: every line expresses how the consumer's agent runs, so there is no useful Octestra-owned remainder to merge. Keeping each file wholly on one side removes a merge protocol and makes update behavior explicit. Changelog entries tell the consumer what policy changes to consider without overwriting their work. Delegating to the downloaded `install.sh` keeps one implementation and means an update runs the new logic rather than whatever the consumer installed months ago. |
...
| D19 | GitHub Action inputs and outputs use `snake_case` | One vocabulary across the public Node action, local composite actions, and step outputs makes workflow wiring mechanically consistent. TypeScript keeps conventional `camelCase` names internally. |
| D20 | `Human Review → Validation` and `Blocked → Validation` are allowed transitions that re-run validation against the task's existing open pull request; re-entering `In Progress` is not allowed | Without these edges, a validation failure or a review round forced the destructive retry: close the pull request, delete the branch, back through `Ready`. Re-entry keeps the status field as the single action surface — a human moves the option and the same `field_added` path runs, so manual and automatic runs never diverge, and no `workflow_dispatch` variant of the lifecycle is needed. An Actions "Re-run" of an old run is not a substitute: the guard compares the event against the live status and rejects it as stale. When no open pull request remains, `prepare-validation` throws and `report-failure` returns the task to `Blocked` loudly. Re-entering `In Progress` was considered and deferred: it needs resume semantics on the existing branch (a resume prompt, owner handling, and a no-branch fallback), while the retry through `Ready` already covers restarting from scratch. |
| D21 | `Todo → In Progress` starts a prepared task directly, without a configuration flag | Moving to `In Progress` explicitly starts implementation. `Ready` queues prepared work but does not guarantee that validation instructions exist: a person can move through it without triage. Repository skills and the person starting a task remain responsible for its preparation, including any required validation instructions. `skip_triage` continues to control loop participation only. |
