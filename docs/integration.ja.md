...
`.github/octestra/config.yml` では、GitHub Actions runner、Octestra が使う GitHub App、task branch の命名規則、prompt template の場所を設定します。

`github_app.client_id`、`runners` 配下の値、または `status.field_id` を変更したあとは、新しい値をリポジトリの Actions 変数へコピーしてください。
